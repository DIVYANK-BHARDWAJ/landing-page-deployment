...
function extendMaterial(BaseMaterial: any, cfg: any) {
  ...
  for (const [inc, code] of Object.entries((cfg.vertex as Record<string, string>) ?? {})) {
    vert = vert.replace(inc, `${inc}\n${code}`);
  }
  for (const [inc, code] of Object.entries((cfg.fragment as Record<string, string>) ?? {})) {
...
function createStackedPlanesBufferGeometry(n: number, width: number, height: number, spacing: number, heightSegments: number) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const uvXOffset = (i * 123.45) % 300;
    const uvYOffset = (i * 456.78) % 300;
